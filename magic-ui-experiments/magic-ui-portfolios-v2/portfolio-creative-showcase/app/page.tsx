...
export default function Portfolio() {
  return (
    <main className="min-h-screen">
      ...
      <section className="min-h-screen flex items-center justify-center px-4 py-20 bg-white">
        ...
        <div className="max-w-6xl mx-auto text-center">
          ...
          <BlurFade delay={0.5} inView>
            <p className="text-3xl md:text-5xl mb-12 text-gray-800">
      ...
      <section className="py-32 px-4 bg-gradient-to-br from-orange-50 to-pink-50">
                ...
                <div className="bg-white/50 dark:bg-gray-800/50 backdrop-blur rounded-lg p-8 shadow-lg">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <p className="text-xl md:text-2xl bg-orange-600 hover:bg-orange-700">{exp.companyName}</p>
                    </div>
                  ...
                  <ul className="space-y-2">
                    {exp.responsibilitiesAndAchievements?.map((item, i) => (
                      <li key={i} className="text-lg md:text-xl text-gray-800">• {item}</li>
                    ))}
                  </ul>
                </div>
              </BlurFade>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-20 space-y-16">
            {cvDataJson.skills.skillCategories.map((category, idx) => (
              <BlurFade key={idx} delay={0.25 * (idx + 1)} inView>
                <div>
                  <h3 className="text-3xl md:text-4xl font-bold mb-8 text-gray-900 border-b-2 border-orange-600 hover:border-orange-700 pb-4">{category.categoryName}</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {category.skills.map((skill, i) => (
                      <div key={i} className="text-2xl md:text-3xl text-gray-800 py-2">
                        • {skill}
                      </div>
                    ))}
                  </div>
            ...
            {cvDataJson.education.educationItems.map((edu, idx) => (
              <BlurFade key={idx} delay={0.25 * (idx + 1)} inView>
                <div className="text-center">
                  ...
                  <p className="text-xl md:text-2xl mb-2 text-gray-800">{edu.institution}</p>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <BlurFade delay={1} inView>
            <div className="mt-12 flex gap-6 justify-center">
              <Button size="lg" className="bg-orange-600 hover:bg-orange-700 text-white text-xl px-8 py-6">
                <Mail className="mr-2 h-5 w-5" />
                Email Me
              </Button>
              <Button size="lg" variant="outline" className="border-2 border-orange-600 hover:border-orange-700 text-gray-900 hover:bg-gray-100 text-xl px-8 py-6">
                Download CV
              </Button>
            </div>
          </BlurFade>
