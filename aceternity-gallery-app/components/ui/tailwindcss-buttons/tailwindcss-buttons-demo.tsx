...
import { ButtonsCard } from "./tailwindcss-buttons-base";
import { ButtonDefinition } from "./tailwindcss-buttons.types";

export function TailwindcssButtons() {
  const copy = (button: ButtonDefinition) => {
    if (button.code) {
...
export function TailwindcssButtonsMinimal() {
  const copy = (button: ButtonDefinition) => {
    if (button.code) {
      copyToClipboard(button.code);
      return;
  ...
  const minimalButtons = buttons.slice(0, 6);

  return (
    <div className="pb-20 px-4 w-full">
      <Toaster position="top-center" />
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full max-w-4xl mx-auto gap-6">
        {minimalButtons.map((button, idx) => (
          <ButtonsCard key={idx} onClick={() => copy(button)}>
            {button.component}
          </ButtonsCard>
        ))}
...
export function TailwindcssButtonsGradient() {
  const copy = (button: ButtonDefinition) => {
    if (button.code) {
      copyToClipboard(button.code);
      return;
  ...
  const gradientButtons = [
    buttons.find(b => b.name === "Gradient"),
    buttons.find(b => b.name === "Lit up borders"),
    buttons.find(b => b.name === "Border Magic"),
    buttons.find(b => b.name === "Shimmer"),
    buttons.find(b => b.name === "Top Gradient"),
    buttons.find(b => b.name === "Tailwindcss Connect"),
  ...
  return (
    <div className="pb-20 px-4 w-full">
      <Toaster position="top-center" />
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full max-w-4xl mx-auto gap-6">
        {gradientButtons.map((button, idx) => (
          <ButtonsCard key={idx} onClick={() => copy(button)}>
            {button.component}
          </ButtonsCard>
        ))}
...
export function TailwindcssButtonsAnimated() {
  const copy = (button: ButtonDefinition) => {
    if (button.code) {
      copyToClipboard(button.code);
      return;
  ...
  const animatedButtons = [
    buttons.find(b => b.name === "Simple"),
    buttons.find(b => b.name === "Invert"),
    buttons.find(b => b.name === "Unapologetic"),
    buttons.find(b => b.name === "Shimmer"),
    buttons.find(b => b.name === "Border Magic"),
    buttons.find(b => b.name === "Figma"),
  ].filter(Boolean) as ButtonDefinition[];
  ...
  return (
    <div className="pb-20 px-4 w-full">
      <Toaster position="top-center" />
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full max-w-4xl mx-auto gap-6">
        {animatedButtons.map((button, idx) => (
          <ButtonsCard key={idx} onClick={() => copy(button)}>
            {button.component}
          </ButtonsCard>
        ))}
