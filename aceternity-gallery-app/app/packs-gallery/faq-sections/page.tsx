...
export default function FAQSectionsGalleryPage() {
  const faqSectionsPack = getPackById("faq-sections");
  const [copiedCode, setCopiedCode] = useState<string | null>(null);
  const variants = faqSectionsPack?.variants || [];
  const [selectedVariant, setSelectedVariant] = useState(variants[0]);

  if (!faqSectionsPack) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p>FAQ Sections pack not found</p>
      </div>
    );
  }
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-4xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
                {faqSectionsPack.title}
              </h1>
            </div>
          </div>

          <p className="text-lg text-gray-300 mb-6 max-w-4xl">
            {faqSectionsPack.description}
          </p>
          ...
          <div className="flex flex-wrap gap-6 mb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-300">{faqSectionsPack.dependencies.length} Dependencies</span>
            </div>
          </div>
        </div>
              ...
              <CardContent className="p-6">
                ...
                <div className="relative">
                  <pre className="bg-slate-900 p-4 rounded-lg text-gray-300 text-sm overflow-x-auto">
                    <code>{faqSectionsPack.installCommand}</code>
                  </pre>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className="absolute top-3 right-3 border-slate-600"
                    onClick={() => copyToClipboard(faqSectionsPack.installCommand, "install")}
                  >
      ...
      0px 0px 0px 1px rgba(25, 28, 33, 0.08);
            ...
            <Card className="bg-slate-800/50 border-slate-700">
              <CardContent className="p-6">
                ...
                <div className="relative">
                  <pre className="bg-slate-900 p-4 rounded-lg text-gray-300 text-sm overflow-x-auto">
                    <code>{faqSectionsPack.importExample}</code>
                  </pre>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className="absolute top-3 right-3 border-slate-600"
                    onClick={() => copyToClipboard(faqSectionsPack.importExample, "import")}
                  >
