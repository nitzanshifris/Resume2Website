...
export default function FootersGalleryPage() {
  const footersPack = getPackById("footers");
  const [copiedCode, setCopiedCode] = useState<string | null>(null);
  const variants = footersPack?.variants || [];
  const [selectedVariant, setSelectedVariant] = useState(variants[0]);

  if (!footersPack) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p>Footers pack not found</p>
      </div>
    );
  }
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-6 mb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-300">{footersPack.dependencies.length} Dependencies</span>
            </div>
          </div>
        </div>
          ...
          <TabsContent value="code" className="space-y-6">
              ...
              <CardContent className="p-6">
                ...
                <div className="relative">
                  <pre className="bg-slate-900 p-4 rounded-lg text-gray-300 text-sm overflow-x-auto">
                    <code>{footersPack.installCommand}</code>
                  </pre>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className="absolute top-3 right-3 border-slate-600"
                    onClick={() => copyToClipboard(footersPack.installCommand, "install")}
                  >
            ...
            <Card className="bg-slate-800/50 border-slate-700">
              <CardContent className="p-6">
                ...
                <div className="relative">
                  <pre className="bg-slate-900 p-4 rounded-lg text-gray-300 text-sm overflow-x-auto">
                    <code>{footersPack.importExample}</code>
                  </pre>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className="absolute top-3 right-3 border-slate-600"
                    onClick={() => copyToClipboard(footersPack.importExample, "import")}
                  >
