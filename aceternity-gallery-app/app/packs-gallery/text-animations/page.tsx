...
import { ArrowLeft, Package, Code2, Eye, Copy, Type } from "lucide-react";
...
export default function TextAnimationsGalleryPage() {
  const textAnimationsPack = getPackById("text-animations");
  const [copiedCode, setCopiedCode] = useState<string | null>(null);
  const variants = textAnimationsPack?.variants || [];
  const [selectedVariant, setSelectedVariant] = useState(variants[0]);

  if (!textAnimationsPack) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p>Text Animations pack not found</p>
      </div>
    );
  }
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-4xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
                {textAnimationsPack.title}
              </h1>
            </div>
          </div>

          <p className="text-lg text-gray-300 mb-6 max-w-4xl">
            {textAnimationsPack.description}
          </p>
          ...
          <div className="flex flex-wrap gap-6 mb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-300">{textAnimationsPack.dependencies.length} Dependencies</span>
            </div>
          </div>
        </div>
          ...
          <TabsContent value="code" className="space-y-6">
              ...
              <CardContent className="p-6">
                ...
                <div className="relative">
                  <pre className="bg-slate-900 p-4 rounded-lg text-gray-300 text-sm overflow-x-auto">
                    <code>{textAnimationsPack.installCommand}</code>
                  </pre>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className="absolute top-3 right-3 border-slate-600"
                    onClick={() => copyToClipboard(textAnimationsPack.installCommand, "install")}
                  >
            ...
            <Card className="bg-slate-800/50 border-slate-700">
              <CardContent className="p-6">
                ...
                <div className="relative">
                  <pre className="bg-slate-900 p-4 rounded-lg text-gray-300 text-sm overflow-x-auto">
                    <code>{textAnimationsPack.importExample}</code>
                  </pre>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className="absolute top-3 right-3 border-slate-600"
                    onClick={() => copyToClipboard(textAnimationsPack.importExample, "import")}
                  >
            ...
            <Card className="bg-slate-800/50 border-slate-700">
              <CardContent className="p-6">
                ...
                <p className="text-gray-300">
                  If you are using Next.js 15 and React 19, please use the following installation instruction for motion package:
                </p>
                <div className="relative mt-4">
                  <pre className="bg-slate-900 p-4 rounded-lg text-gray-300 text-sm overflow-x-auto">
                    <code>npm i motion --legacy-peer-deps</code>
                  </pre>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className="absolute top-3 right-3 border-slate-600"
                    onClick={() => copyToClipboard("npm i motion --legacy-peer-deps", "motion-legacy")}
                  >
                    {copiedCode === "motion-legacy" ? "Copied!" : <Copy className="h-4 w-4" />}
                  </Button>
                </div>
              </CardContent>
            </Card>
