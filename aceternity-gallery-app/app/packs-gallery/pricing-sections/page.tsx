...
export default function PricingSectionsGalleryPage() {
  const pricingSectionsPack = getPackById("pricing-sections");
  const [copiedCode, setCopiedCode] = useState<string | null>(null);
  const variants = pricingSectionsPack?.variants || [];
  const [selectedVariant, setSelectedVariant] = useState(variants[0]);

  if (!pricingSectionsPack) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p>Pricing Sections pack not found</p>
      </div>
    );
  }
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-4xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
                {pricingSectionsPack.title}
              </h1>
            </div>
          </div>

          <p className="text-lg text-gray-300 mb-6 max-w-4xl">
            {pricingSectionsPack.description}
          </p>
          ...
          <div className="flex flex-wrap gap-6 mb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-300">{pricingSectionsPack.dependencies.length} Dependencies</span>
            </div>
          </div>
        </div>
              ...
              <Card className="bg-slate-800/50 border-slate-700">
                <CardContent className="p-6">
                  ...
                  <div className="relative rounded-lg border border-slate-700 overflow-hidden">
                    <div className="min-h-[600px] lg:min-h-[800px] bg-white dark:bg-gray-900">
          ...
          <TabsContent value="code" className="space-y-6">
              ...
              <CardContent className="p-6">
                ...
                <div className="relative">
                  <pre className="bg-slate-900 p-4 rounded-lg text-gray-300 text-sm overflow-x-auto">
                    <code>{pricingSectionsPack.installCommand}</code>
                  </pre>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className="absolute top-3 right-3 border-slate-600"
                    onClick={() => copyToClipboard(pricingSectionsPack.installCommand, "install")}
                  >
            ...
            <Card className="bg-slate-800/50 border-slate-700">
              <CardContent className="p-6">
                ...
                <div className="relative">
                  <pre className="bg-slate-900 p-4 rounded-lg text-gray-300 text-sm overflow-x-auto">
                    <code>{pricingSectionsPack.importExample}</code>
                  </pre>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className="absolute top-3 right-3 border-slate-600"
                    onClick={() => copyToClipboard(pricingSectionsPack.importExample, "import")}
                  >
