...
export function FAQsWithGrid() {
  const [open, setOpen] = useState<string | null>(null);
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-20 md:px-8 md:py-40">
      <h2 className="mb-4 text-center text-4xl font-medium tracking-tight text-neutral-600 md:text-5xl dark:text-neutral-50">
        Frequently asked questions
      </h2>
      <p className="mx-auto mb-12 max-w-lg text-center text-base text-neutral-600 dark:text-neutral-50">
        We are here to help you with any questions you may have.
      </p>
      <div className="mx-auto grid max-w-5xl grid-cols-1 gap-x-20 gap-y-12 md:grid-cols-2">
        {FAQs.map((faq, index) => (
          <FAQItemMinimal
            key={index}
            question={faq.question}
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <div className="ml-4">
          <IconPlus
            className={cn(
              "h-5 w-5 transform text-blue-500 transition-all duration-200",
              isOpen && "rotate-45",
            )}
          />
        </div>
      ...
      <AnimatePresence mode="wait">
        {isOpen && (
          <motion.div
            ...
            transition={{ duration: 0.2, ease: "easeOut" }}
            className="mt-2 overflow-hidden text-neutral-500 dark:text-neutral-400"
          >
            <p>{answer}</p>
          </motion.div>
        )}
