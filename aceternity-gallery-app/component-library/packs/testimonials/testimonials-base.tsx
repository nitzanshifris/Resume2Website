...
export function TestimonialsMarqueeGrid() {
  return (
    <div className="relative w-full max-w-7xl mx-auto px-4 md:px-8 py-12">
      <div className="pb-12">
        <h1 className="font-bold text-black text-lg md:text-2xl dark:text-white">
          Used by builders around the world
        </h1>
...
export const TestimonialsMarquee = () => {
  ...
  return (
    <div className="relative space-y-6 [mask-image:linear-gradient(to_right,transparent_0%,white_10%,white_90%,transparent_100%)]">
      <Marquee direction="right" pauseOnHover speed={30} gradient={false}>
        {first.map((testimonial, index) => (
          <MarqueeCard key={`testimonial-${testimonial.src}-${index}`}>
            <MarqueeQuote>{testimonial.quote}</MarqueeQuote>
            <div className="flex gap-2 items-center mt-4">
              ...
              />
              <div className="flex flex-col">
                <MarqueeQuoteDescription className="text-neutral-700 dark:text-neutral-300 font-medium">
                  {testimonial.name}
                </MarqueeQuoteDescription>
                <MarqueeQuoteDescription className="text-neutral-500 dark:text-neutral-400 text-xs">
                  {testimonial.designation}
                </MarqueeQuoteDescription>
              </div>
            </div>
      ...
      <Marquee direction="left" pauseOnHover speed={40} gradient={false}>
...
export const MarqueeCard = ({
  ...
  return (
    <div
      className={cn(
        "p-6 rounded-xl h-auto w-[350px] mx-4 bg-gray-50 dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 shadow-sm",
        className
      )}
    >
      {children}
...
export const MarqueeQuoteDescription = ({
  children,
  className,
}: {
  children: React.ReactNode;
    ...
    <div className="py-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 max-w-7xl mx-auto items-start px-4 md:px-8 mt-10">
        {grid.map((testimonialsCol, index) => (
          ...
          >
            {testimonialsCol.map((testimonial) => (
              <MasonryCard key={`testimonial-${testimonial.src}-${index}`}>
                <MasonryQuote>{testimonial.quote}</MasonryQuote>
                <div className="flex gap-2 items-center mt-8">
                  <Image
                    src={testimonial.src}
                    alt={testimonial.name}
                  ...
                  />
                  <div className="flex flex-col">
                    <MasonryQuoteDescription>{testimonial.name}</MasonryQuoteDescription>
                    <MasonryQuoteDescription className="text-[10px]">
                      {testimonial.designation}
                    </MasonryQuoteDescription>
                  </div>
                </div>
              </MasonryCard>
            ))}
...
export const MasonryQuoteDescription = ({
  children,
  className,
}: {
  children: React.ReactNode;
