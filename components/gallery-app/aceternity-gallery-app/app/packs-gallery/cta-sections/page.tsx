...
import { 
  ...
  Megaphone, Star, Download, BookOpen, Settings
} from "lucide-react";
import Link from "next/link";
import { getPackById } from "@/component-library/packs";

// Get the CTA sections pack from registry
const ctaSectionsPack = getPackById("cta-sections");
const ctaVariants = ctaSectionsPack?.variants || [];
const dependencies = ctaSectionsPack?.dependencies || [];

export default function CTASectionsPackPage() {
  const [selectedVariant, setSelectedVariant] = useState(ctaVariants[0]);
  ...
  const installCommand = ctaSectionsPack?.installCommand || `npm i ${dependencies.join(" ")}`;
  const importCode = ctaSectionsPack?.importExample || `import { /* CTA components */ } from "@/packs/cta-sections";`;
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-4xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
                {ctaSectionsPack?.title || "CTA Sections"}
              </h1>
            </div>
          </div>

          <p className="text-lg text-gray-300 mb-6 max-w-4xl">
            {ctaSectionsPack?.description || "CTA sections with modern and minimalist styles. Perfect for conversion-focused landing pages."}
          </p>
          ...
          <div className="flex flex-wrap gap-6 mb-8">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-300">{ctaVariants.length} Variants</span>
            </div>
            <div className="flex items-center gap-2">
              <Star className="h-5 w-5 text-yellow-400" />
              <span className="text-gray-300">{ctaVariants.filter(v => v.featured).length} Featured</span>
            </div>
            <div className="flex items-center gap-2">
            ...
            {ctaVariants.length === 0 ? (
              <Card className="bg-slate-800/50 border-slate-700">
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-gray-500">CTA section components will be added here soon.</p>
                </CardContent>
              </Card>
            ) : (
              <>
