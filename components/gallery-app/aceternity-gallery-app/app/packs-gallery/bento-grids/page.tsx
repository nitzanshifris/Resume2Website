...
import { 
  ...
  Grid3x3, Star, Download, BookOpen, Settings
} from "lucide-react";
import Link from "next/link";
import { getPackById } from "@/component-library/packs";

// Get the bento grids pack from registry
const bentoGridsPack = getPackById("bento-grids");
const bentoVariants = bentoGridsPack?.variants || [];
const dependencies = bentoGridsPack?.dependencies || [];

export default function BentoGridsPackPage() {
  const [selectedVariant, setSelectedVariant] = useState(bentoVariants[0]);
  ...
  const installCommand = bentoGridsPack?.installCommand || `npm i ${dependencies.join(" ")}`;
  const importCode = bentoGridsPack?.importExample || `import { /* Bento Grid components */ } from "@/packs/bento-grids";`;
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-4xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
                {bentoGridsPack?.title || "Bento Grids"}
              </h1>
            </div>
          </div>

          <p className="text-lg text-gray-300 mb-6 max-w-4xl">
            {bentoGridsPack?.description || "A set of bento grids for various use cases. Modern grid layouts with interactive elements and smooth animations."}
          </p>
          ...
          <div className="flex flex-wrap gap-6 mb-8">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-300">{bentoVariants.length} Variants</span>
            </div>
            <div className="flex items-center gap-2">
              <Star className="h-5 w-5 text-yellow-400" />
              <span className="text-gray-300">{bentoVariants.filter(v => v.featured).length} Featured</span>
            </div>
            <div className="flex items-center gap-2">
            ...
            {bentoVariants.length === 0 ? (
              <Card className="bg-slate-800/50 border-slate-700">
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-gray-500">Bento grid components will be added here soon.</p>
                </CardContent>
              </Card>
            ) : (
              <>
