...
export default function CardsGalleryPage() {
  const cardsPack = getPackById("cards");
  const [copiedCode, setCopiedCode] = useState<string | null>(null);
  const variants = cardsPack?.variants || [];
  const [selectedVariant, setSelectedVariant] = useState(variants[0]);

  if (!cardsPack) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p>Cards pack not found</p>
      </div>
    );
  }
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-6 mb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-300">{cardsPack.dependencies.length} Dependencies</span>
            </div>
          </div>
        </div>
              ...
              <Card className="bg-slate-800/50 border-slate-700">
                <CardContent className="p-6">
                  ...
                  <div className="relative rounded-lg border border-slate-700 overflow-hidden">
                    <div className="min-h-[400px] bg-white dark:bg-gray-900 flex items-center justify-center p-8">
          ...
          <TabsContent value="code" className="space-y-6">
              ...
              <CardContent className="p-6">
                ...
                <div className="relative">
                  <pre className="bg-slate-900 p-4 rounded-lg text-gray-300 text-sm overflow-x-auto">
                    <code>{cardsPack.installCommand}</code>
                  </pre>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className="absolute top-3 right-3 border-slate-600"
                    onClick={() => copyToClipboard(cardsPack.installCommand, "install")}
                  >
            ...
            <Card className="bg-slate-800/50 border-slate-700">
              <CardContent className="p-6">
                ...
                <div className="relative">
                  <pre className="bg-slate-900 p-4 rounded-lg text-gray-300 text-sm overflow-x-auto">
                    <code>{cardsPack.importExample}</code>
                  </pre>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className="absolute top-3 right-3 border-slate-600"
                    onClick={() => copyToClipboard(cardsPack.importExample, "import")}
                  >
