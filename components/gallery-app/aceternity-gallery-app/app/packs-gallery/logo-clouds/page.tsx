...
import { 
  ...
  Cloud, Shuffle, Star, Download, BookOpen, Settings
} from "lucide-react";
import Link from "next/link";
import { getPackById } from "@/component-library/packs";

// Get the logo clouds pack from registry
const logoCloudsPack = getPackById("logo-clouds");
const logoVariants = logoCloudsPack?.variants || [];
const dependencies = logoCloudsPack?.dependencies || [];

export default function LogoCloudsPackPage() {
  const [selectedVariant, setSelectedVariant] = useState(logoVariants[0]);
  ...
  const installCommand = logoCloudsPack?.installCommand || `npm i ${dependencies.join(" ")}`;
  const importCode = logoCloudsPack?.importExample || `import { ${logoVariants.map(v => v.component.name).join(", ")} } from "@/packs/logo-clouds";`;
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-4xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
                {logoCloudsPack?.title || "Logo Clouds"}
              </h1>
            </div>
          </div>

          <p className="text-lg text-gray-300 mb-6 max-w-4xl">
            {logoCloudsPack?.description || "A collection of logo clouds with micro interactions and minimal animations. Perfect for showcasing client logos, partner brands, or technology stacks with smooth transitions and effects."}
          </p>
          ...
          <div className="flex flex-wrap gap-6 mb-8">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-300">{logoVariants.length} Variants</span>
            </div>
            <div className="flex items-center gap-2">
              <Star className="h-5 w-5 text-yellow-400" />
              <span className="text-gray-300">{logoVariants.filter(v => v.featured).length} Featured</span>
            </div>
            <div className="flex items-center gap-2">
