...
import { Mouse, Layers, Package, Sparkles } from 'lucide-react'

export default function HomePage() {
  return (
    <div className="min-h-[calc(100vh-3.5rem)] flex items-center justify-center">
      {/* Animated background shapes – pointer-events disabled so they don't block nav clicks */}
      ...
      <div className="text-center relative z-10 px-4">
        ...
        <div className="flex gap-4 justify-center flex-wrap">
          ...
          <Link 
            ...
            className="group relative inline-flex items-center gap-2 px-6 py-3 text-white rounded-lg transition-all duration-300 bg-gray-900 hover:bg-gray-800 border border-gray-800 hover:border-gray-700 glow-effect shimmer-effect overflow-hidden"
          >
            <div className="absolute inset-0 bg-gradient-to-r from-teal-600/20 to-cyan-600/20 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity blur-xl" />
            <Sparkles className="w-4 h-4 relative z-10 group-hover:scale-110 transition-transform duration-300" />
            <span className="relative z-10">Dynamic Playground</span>
          </Link>
          <Link 
            href="/packs-gallery"
            className="group relative inline-flex items-center gap-2 px-6 py-3 text-white rounded-lg transition-all duration-300 bg-gray-900 hover:bg-gray-800 border border-gray-800 hover:border-gray-700 glow-effect shimmer-effect overflow-hidden"
