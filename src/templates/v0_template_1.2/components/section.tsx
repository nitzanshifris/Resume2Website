...
export function Section({ id, title, onSaveTitle, isVisible, className, children, fullWidth = false }: SectionProps) {
  return (
    <AnimatePresence>
      {isVisible && (
        <motion.section id={id} {...sectionAnimation} className={cn("py-16", className)}>
          <div className={cn(!fullWidth && "max-w-5xl mx-auto px-4")}>
            <EditableText
              as="h2"
              initialValue={title}
              onSave={onSaveTitle}
              className="text-6xl font-bold text-center mb-12 font-serif text-glow max-w-5xl mx-auto px-4"
            />
            {children}
          </div>
        </motion.section>
