...
import { Users, Medal, Trophy, Code2, Database, Cloud, GitBranch, Shield, Cpu } from "lucide-react"
...
const getSkillIcon = (categoryName) => {
  const icons = {
    ...
    "Frontend Development": IconBrandInstagram,
    "Cloud & DevOps": Cloud,
...
export const mapCvDataToSections = (cvData) => {
  ...
  if (cvData.certifications?.certificationItems?.length > 0) {
    sections.push({
      ...
      data: cvData.certifications.certificationItems.map((item) => {
        // Check if the issuing organization is already mentioned in the title
        const titleLower = item.title?.toLowerCase() || '';
        const orgLower = item.issuingOrganization?.toLowerCase() || '';
        ...
        // Check if org or its abbreviation is already in the title
        let showSubtitle = true;
        if (orgLower) {
          // Check direct match
          if (titleLower.includes(orgLower)) {
            showSubtitle = false;
          }
          // Check abbreviation matches
          for (const [abbr, fullName] of Object.entries(orgMappings)) {
            if ((titleLower.includes(abbr) && orgLower.includes(fullName)) ||
                (titleLower.includes(fullName) && orgLower.includes(abbr))) {
              showSubtitle = false;
              break;
            }
          }
        ...
        return {
          title: item.title,
          subtitle: showSubtitle ? item.issuingOrganization : '',
          date: item.issueDate,
        };
      }),
    })
  ...
  if (cvData.achievements?.achievements?.length > 0) {
    sections.push({
      ...
      data: cvData.achievements.achievements.map((achievement: any) => {
        // If achievement is a string, convert to object format
        if (typeof achievement === 'string') {
          return {
            title: achievement.split(' - ')[0] || achievement,
            description: achievement.split(' - ')[1] || achievement
          }
        }
        // If it's already an object with label/value, convert to title/description
        if (achievement.label && achievement.value) {
          let title = achievement.value
          let description = achievement.label
          if (achievement.contextOrDetail) {
            description += ` ${achievement.contextOrDetail}`
          }
          if (achievement.timeframe) {
            description += ` (${achievement.timeframe})`
          }
          return {
            title: title,
        ...
        // Otherwise return as is (assuming it already has title/description)
        return achievement
      }),
    })
  }
