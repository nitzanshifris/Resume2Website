  ...
  useEffect(() => {
    const loadCVData = async () => {
      try {
        ...
        // Check if we're in standalone mode (port 9999 or development ports 3020/3021)
        const isStandalone = window.location.port === '9999' || window.location.port === '3020' || window.location.port === '3021'
        
        if (isStandalone) {
          // For development/standalone, try to load injected data first
          ...
          try {
            ...
            if (injectedModule.useRealData && injectedModule.portfolioData) {
              console.log('✅ Using injected CV data (mock data for testing)')
              setData(injectedModule.portfolioData)
              toast.success('Portfolio loaded with mock CV data!')
              return // Exit early - we have the data
            }
          } catch (injectedError) {
            console.log('ℹ️ No injected data found, using demo data')
          ...
          // For generated portfolios in production
          console.log('🔄 Loading CV data...')
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              {data.experience.experienceItems.map((item, i) => (
                <SmartCard
                  ...
                  item={item}
                  onUpdate={(field, value) => handleSave(`experience.experienceItems.${i}.${field}`, value)}
