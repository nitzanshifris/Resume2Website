              ...
              {data.professionalLinks.map((link, index) => (
                <div key={link.name} className="relative group">
                  <a
                    ...
                    className={cn(
                      "p-3 bg-card/50 rounded-full text-foreground hover:bg-accent hover:text-accent-foreground transition-all duration-300 ease-in-out transform hover:scale-110 backdrop-blur-sm border border-border/20 block",
                      isEditMode && "gradient-border-dashed"
                    )}
                    aria-label={link.name}
                  >
                    {isEditMode && safeOnSaveProfessionalLink ? (
                      <IconWithZoom
                        icon={link.customIcon || link.name}
                        onIconUpdate={(newIcon) => {
                          safeOnSaveProfessionalLink(index, "customIcon", newIcon)
                        }}
