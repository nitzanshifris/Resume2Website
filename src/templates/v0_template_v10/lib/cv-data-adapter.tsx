  ...
  return {
    ...
    experience: {
      ...
      experienceItems: (cv2webData.experience?.experienceItems || []).map(item => ({
        ...
        // Add technologies array (mapped from technologiesUsed)
        technologies: item?.technologiesUsed || [],
        // Keep responsibilities as array for optional bullet display
        responsibilities: item?.responsibilitiesAndAchievements || [],
        // Add additional fields for badges
        isCurrent: item?.dateRange?.isCurrent || false,
  ...
  try {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        console.log('No injected data module found')
      }
    }
    
    // Check if we're in development/standalone mode
    const port = typeof window !== 'undefined' ? window.location.port : ''
    if (port === '3020' || port === '3021' || port === '9999') {
      console.log('📋 Development mode - using mock/injected data')
      // Try to return injected data if available
      try {
