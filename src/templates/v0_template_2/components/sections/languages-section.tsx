        ...
        {items.map((lang, i) => (
          ...
          >
            ...
            <div className="relative">
              <LanguagesIcon className="h-8 w-8 text-accent flex-shrink-0" />
              <div>
                <h3 className={cn("text-neutral-100 font-bold", getSizeClasses("timelineTitle"))}>{lang.name}</h3>
                <p className={cn("text-neutral-400", getSizeClasses("timelineSubtitle"))}>{lang.proficiency}</p>
              </div>
            </div>
          </motion.div>
        ))}
