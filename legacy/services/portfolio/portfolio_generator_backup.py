...
class PortfolioGenerator:
    ...
    def _generate_main_page(self, selections: List[ComponentSelection], user_name: str) -> str:
        ...
        for selection in selections:
            ...
            # Add import with correct file path
            base_file = component_file_mapping.get(selection.component_type, selection.component_type)
            if selection.component_type == 'bento-grid':
                imports.add(f"import {{ BentoGrid, BentoGridItem }} from '@/components/ui/{selection.component_type}/{base_file}';")
            elif selection.component_type == 'card-hover-effect':
                imports.add(f"import {{ HoverEffect }} from '@/components/ui/{selection.component_type}/{base_file}';")
                component_name = 'HoverEffect'  # Fix component name
            else:
                imports.add(f"import {{ {component_name} }} from '@/components/ui/{selection.component_type}/{base_file}';")
            ...
            # Generate component usage based on ALL component types
...
{dock_mapping}
  ...
  return (
    ...
    def _render_component_tsx_with_adapter(self, component_type: str, component_name: str,
      ...
      <section id="{section}" className="py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <{component_name} {prop_name}={{{props_name}.{prop_name} || {{{props_name}.words || '{user_name}'}}}} className="text-center" />
        </div>
      </section>'''
      ...
      <section id="{section}" className="py-20">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-white">{section_title}</h2>
    ...
    def _generate_fallback_component(self, component_type: str, component_name: str, 
      ...
      <section id="{section}" className="relative">
        <{component_name} products={{{props_name}.products || []}} />
        ...
        elif component_type in ['background-gradient', 'background-beams', 'aurora-background']:
            # Wrapper components
            return f"""
      {{/* Hero Section */}}
      <section className="min-h-screen flex items-center justify-center relative p-8">
        <{component_name} className="absolute inset-0 pointer-events-none" />
        <div className="relative z-10 text-center max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-7xl font-bold text-white mb-4">
            {{{props_name}.title || 'Portfolio'}}
          </h1>
          <p className="text-xl md:text-2xl text-gray-300">
            {{{props_name}.subtitle || ''}}
          </p>
          {{{props_name}.description && (
            ...
            return f"""
      {{/* {section.title()} Section with {component_type.replace('-', ' ').title()} */}}
      <section id="{section}" className="relative">
        <{component_name} className="min-h-[40rem]">
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center px-4">
              <h1 className="text-4xl md:text-7xl font-bold text-center bg-clip-text text-transparent bg-gradient-to-b from-neutral-50 to-neutral-400">
                {{{props_name}.title || '{user_name}'}}
      ...
      <section id="{section}" className="py-20 px-4">
        <div className="max-w-4xl mx-auto">
          <{component_name} {prop_name}={{{props_name}.{prop_name} || ''}} className="text-center" />
        </div>
      </section>"""
      ...
      <section id="{section}" className="py-20">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-white">{section.title()}</h2>
          <{component_name} data={{{props_name}.data || []}} />
        </div>
      </section>"""
      
        # Grid components
        elif component_type in ['bento-grid', 'layout-grid']:
            if section == 'skills':
                return f"""
      {{/* Skills Grid */}}
      {{skillItems.length > 0 && (
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-white">{section.title()}</h2>
          <{component_name} className="max-w-5xl mx-auto">
            {{{props_name}.items?.map((item: any, i: number) => (
              <BentoGridItem
                key={{i}}
                title={{item.title}}
      ...
      {{{props_name}.items && {props_name}.items.length > 0 && (
        <section className="py-20">
          <div className="max-w-7xl mx-auto px-4">
            <h2 className="text-3xl font-bold text-center mb-12 text-white">{section.title()}</h2>
            <HoverEffect items={{{props_name}.items}} />
          </div>
        </section>
      )}}"""
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <{component_name} items={{{props_name}.items || []}} />
          </div>
        </div>
      </section>"""
      ...
      <section id="{section}" className="py-20">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-white">{section.title()}</h2>
          <{component_name} testimonials={{{props_name}.testimonials || []}} autoplay={{true}} />
        </div>
      </section>"""
      ...
      {{dockItems.length > 0 && (
        ...
        # Moving/animated cards
        elif component_type == 'infinite-moving-cards':
            return f"""
      {{/* {section.title()} - Moving Cards */}}
      <section id="{section}" className="py-20">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-white">{section.title()}</h2>
          <{component_name} 
        ...
        # Parallax scroll
        elif component_type == 'parallax-scroll':
            return f"""
      {{/* {section.title()} - Parallax Gallery */}}
      <section id="{section}" className="py-20">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-white">{section.title()}</h2>
          <{component_name} images={{{props_name}.images || []}} className="max-w-6xl mx-auto" />
        </div>
      </section>"""
      
        # 3D components
        elif component_type in ['3d-card', '3d-pin']:
            return f"""
      {{/* {section.title()} - 3D Cards */}}
      <section id="{section}" className="py-20">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-white">{section.title()}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {{{props_name}.items?.map((item: any, i: number) => (
              <{component_name} key={{i}} {{...item}} />
            ))}}
          </div>
        </div>
      </section>"""
        ...
        elif component_type == 'sticky-scroll-reveal':
            return f"""
      {{/* {section.title()} - Scroll Reveal */}}
      <section id="{section}" className="py-20">
        <{component_name} content={{{props_name}.content || []}} />
      </section>"""
      
        # Focus cards
        elif component_type == 'focus-cards':
            return f"""
      {{/* {section.title()} - Focus Cards */}}
      <section id="{section}" className="py-20">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-white">{section.title()}</h2>
          <{component_name} cards={{{props_name}.cards || []}} />
        </div>
      </section>"""
      
        # Animated tooltip
        elif component_type == 'animated-tooltip':
            return f"""
      ...
      <section id="{section}" className="py-20">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {{{props_name}.items?.map((item: any, i: number) => (
              <{component_name} key={{i}} items={{[item]}} />
            ))}}
          </div>
        </div>
      </section>"""
        ...
        # Generic fallback for any other component
            ...
            # Check if it needs items, data, children, etc.
            if "items" in required_props:
                return f"""
      {{/* {section.title()} Section */}}
      <section id="{section}" className="py-20">
        <div className="max-w-7xl mx-auto px-4">
      ...
      <section id="{section}" className="py-20">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <{component_name} data={{{props_name}.data || []}} />
            ...
            elif "children" in required_props:
                return f"""
      {{/* {section.title()} Section */}}
      <section id="{section}" className="py-20">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-white">{section.title()}</h2>
          <{component_name}>
            <div dangerouslySetInnerHTML={{{{ __html: {props_name}.content || '' }}}} />
          </{component_name}>
        </div>
      </section>"""
            else:
                # Default spread all props
                return f"""
      {{/* {section.title()} Section */}}
      <section id="{section}" className="py-20">
        <div className="max-w-7xl mx-auto px-4">
  ...
  return (
    ...
    def _generate_data_file(self, selections: List[ComponentSelection]) -> str:
        ...
        for selection in selections:
            ...
            props_str = json.dumps(selection.props, indent=2)
...
"""
    ...
    def _fix_component_imports(self, output_dir: Path) -> None:
        """Fix import paths in copied Aceternity components"""
        components_dir = output_dir / "components" / "ui"
        ...
        try:
            ...
            logger.info("Fixed import paths in Aceternity components")
