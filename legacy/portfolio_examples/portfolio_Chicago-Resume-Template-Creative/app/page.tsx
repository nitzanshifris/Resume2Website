...
import { FlipWords } from '@/components/ui/flip-words/flip-words-base';
...
import { Timeline } from '@/components/ui/timeline/timeline-base';
...
export default function Portfolio() {
  ...
  const volunteerData = portfolioData.volunteer || {};
  const speakingData = portfolioData.speaking || {};
...
<section id="hero" className="min-h-screen flex items-center justify-center relative">
      ...
      {(summaryData.words || summaryData.text || summaryData.content) && (
        <section id="summary" className="py-20 px-4">
          <div className="max-w-4xl mx-auto text-center">
            <TextGenerateEffect words={summaryData.words || 'EMMA WILSON'} className="text-center" />
          </div>
        </section>
      )}
      ...
      {(languagesData.items?.length > 0 || languagesData.cards?.length > 0 || languagesData.entries?.length > 0 || languagesData.people?.length > 0 || languagesData.testimonials?.length > 0) && (
        <section id="languages" className="py-20">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <AnimatedTooltip 
              items={languagesData.people || languagesData.items || []}
            />
          </div>
        </section>
      )}
      ...
      {/* Volunteer Cards */}
      {(volunteerData.items?.length > 0 || volunteerData.cards?.length > 0 || volunteerData.entries?.length > 0 || volunteerData.people?.length > 0 || volunteerData.testimonials?.length > 0) && (
        <section className="py-20">
          <div className="max-w-7xl mx-auto px-4">
            <h2 className="text-3xl font-bold text-center mb-12 text-white">Volunteer</h2>
            <HoverEffect items={volunteerData.cards || volunteerData.items || []} />
          </div>
        </section>
      )}

      {/* Speaking Timeline */}
      {(speakingData.items?.length > 0 || speakingData.cards?.length > 0 || speakingData.entries?.length > 0 || speakingData.people?.length > 0 || speakingData.testimonials?.length > 0) && (
        <section id="speaking" className="py-20">
          <div className="max-w-7xl mx-auto px-4">
            <h2 className="text-3xl font-bold text-center mb-12 text-white">Speaking</h2>
            <Timeline data={speakingData.entries || []} />
          </div>
        </section>
      )}

      {/* Hobbies Section */}
      {(hobbiesData.words || hobbiesData.text || hobbiesData.content) && (
        <section id="hobbies" className="py-20 px-4">
          <div className="max-w-4xl mx-auto text-center">
            <FlipWords words={hobbiesData.words || 'EMMA WILSON'} className="text-center" />
          </div>
        </section>
      )}
