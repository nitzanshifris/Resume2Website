  ...
  useEffect(() => {
    const handleFacebookCallback = async () => {
      try {
        ...
        if (error) {
          setStatus('error');
          setMessage(errorDescription || 'Facebook authentication was cancelled or failed.');
          return;
        }
        ...
        const response = await fetch(`${apiUrl}/api/v1/auth/facebook/callback`, {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          if (errorMessage.includes('Facebook OAuth is not configured')) {
            setMessage('Facebook Sign-In is temporarily unavailable. Please use email/password registration instead.');
          } else {
