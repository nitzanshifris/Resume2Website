...
describe('Upload Flow E2E Tests', () => {
  ...
  describe('Anonymous User Tests', () => {
    test('Non-resume image → 400 "Not a resume", no animation, toast persists', async () => {
    ...
    test('Valid PDF triggers auth modal for anonymous users', async () => {
  ...
  describe('Authenticated User Tests', () => {
    ...
    test('Valid PDF → 200, CV appears, animation starts post-validation', async () => {
      // Upload valid PDF
      const fileInput = await page.locator('input[type="file"]')
      await fileInput.setInputFiles(VALID_PDF_RESUME)
  ...
  describe('Error Code Tests', () => {
    test('401/403 → Auth modal opens, no toast', async () => {
      // Mock 401 response
    ...
    test('413 → "File is too large" with correct copy', async () => {
      // Mock 413 response
      await page.route('**/api/v1/upload', route => {
        route.fulfill({
          status: 413,
    ...
    test('429 → "Too many uploads" with correct copy', async () => {
      // Mock 429 response
      await page.route('**/api/v1/upload', route => {
        route.fulfill({
          status: 429,
  ...
  describe('Network Error Tests', () => {
    test('Network down → "Connection problem" message', async () => {
    ...
    test('Timeout → "Upload timed out" message', async () => {
      // Simulate timeout
  ...
  describe('Retry Flow Tests', () => {
    test('Retry replaces CV immediately, validation-first', async () => {
