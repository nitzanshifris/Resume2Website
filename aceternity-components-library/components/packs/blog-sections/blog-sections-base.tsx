...
// Simple Container component for blog sections
  ...
  return (
    <div className="relative overflow-hidden">
      <Container className="flex flex-col items-center justify-between pb-20">
        ...
        {blogsWithDates.slice(0, 1).map((blog, index) => (
          <BlogCardWithDate blog={blog} key={blog.title + index} />
        ))}

        <BlogPostRows blogs={blogsWithDates} />
      </Container>
    </div>
  );
}

export const BlogPostRows = ({ blogs }: { blogs: BlogWithDate[] }) => {
...
export const BlogPostRow = ({ blog }: { blog: BlogWithDate }) => {
  return (
    <Link
      href={`${blog.slug}`}
      key={`${blog.slug}`}
...
export const BlogCardWithDate = ({ blog }: { blog: BlogWithDate }) => {
  return (
    <Link
...
// Blog type with date for search functionality
type BlogWithDate = {
  title: string;
  description: string;
  date: string;
...
const blogsWithDates: BlogWithDate[] = [
  {
    title: "Changelog for 2024",
    description:
      "Explore the latest updates and enhancements in our 2024 changelog. Discover new features and improvements that enhance user experience.",
